    ...
    <div class="customTable">
        <div class="table-center">
            <table class="table table-hover border border-dark mt-3 table-bordered">
                ...
                <#list rents as rent >
                    <tr id="values">
                        <td class="${rent.id}" id="id">${rent.id}</td>
                        <td class="${rent.id}" id="name">${rent.name}</td>
                        <td class="${rent.id}" id="description">${rent.description}</td>
                        <td class="${rent.id}" id="createdAt">${rent.createdAt}</td>
                        <td class="${rent.id}" id="updatedAt">${rent.updatedAt?if_exists}</td>
                        <td class="${rent.id}" id="startDate">${rent.startDate}</td>
                        <td class="${rent.id}" id="endDate">${rent.endDate}</td>
                        <td><a><button type="button" class="btn update" onclick=onClickUpdate("${rent.id}")><ion-icon name="create" style="color: sandybrown;"></ion-icon></button></a></td>
                        <td><a href="/ui/v1/rents/del/${rent.id}"><button type="button" class="btn"><ion-icon name="trash" style="color: orangered;"></ion-icon></button></a></td>
                    </tr>
                </#list>
                </tbody>
            </table>
...
<div id="myModalAdd" class="customModal">
    ...
    <div class="customModal-content">
        <fieldset>
            <form action="/ui/v1/rents/add" method="post">
                <div class="modal-body">
                    ...
                    <p>Start Date: <input type="datetime-local" name="startDate" class="form-control"/></p>
                    <p>End Date: <input type="datetime-local" name="endDate" class="form-control"/></p>
                </div>
...
<div id="myModalUpdate" class="customModal">
    ...
    <div class="customModal-content">
        <fieldset>
            <form action="/ui/v1/rents/edit/" method="post" id="updateForm">
